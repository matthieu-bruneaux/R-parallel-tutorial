...
its setup is quite simple. The approach described here with \texttt{snow}
works well with a personal computer or when using a single node on Taito
(each Taito node contains 16 cores), but for analyses using multiple nodes
(e.g. 32 or 64 cores) we will use the \texttt{Rmpi} package.
...
\texttt{parApply} parallelized functions.
...
the list object which is very versatile but might seem a bit daunting to users
without any prior experience with it.

Using the \texttt{apply} family of functions tends to produce cleaner and
better code - cleaner since the user has to structure the data efficiently into
...
Running code which already uses any \texttt{apply} family function on multiple
