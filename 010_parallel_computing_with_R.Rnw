...
One quick-and-dirty way for using several cores is to manually start as many
...
analysis to 16, 32 or more cores on a computer grid, manually starting and 
...
\begin{itemize}
  ...
  detect regions with significantly high Gst on each chromosome. The analyses 
  for each chromosome are independent and can be performed on separate cores.
...
used by R users when they just start to learn R.
