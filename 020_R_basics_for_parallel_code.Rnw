...
data stored as list and the \texttt{lapply} approach. It covers list
creation and manipulation and \texttt{lapply} call.
...
\section{Using \texttt{sapply} and \texttt{lapply} on a vector and a list}
%-------------------------------------------------------------------------
...
In the next section, we will see how to store more meaningful examples of how to
store data into a list from an existing dataset and of using \texttt{lapply}.
...
When a data set is loaded from a text file (e.g. produced by Excel), it is 
usually stored as a \emph{data frame}. \texttt{iris} is a dataset shipped with R that 
we will use in this short example. It is a data frame, which is equivalent to a 
table containing data arranged in columns. Each row represents a record and each column represents a variable.
...
of rows or columns. First we can store in a vector (\texttt{long.sepal}) whether
or not the sepal length of the records are less than 5:
...
\texttt{apply} is the apply family function that can be used to apply one 
...
<<>>=
head(iris[, 1:4]) # we use only the first four columns
apply(iris[, 1:4], 2, mean) # 2 specifies to work on columns, and mean
                     # is the function we apply to each column
apply(iris[, 1:4], 2, sd) # sd calculates standard deviation
@
...
\section{Arranging data into a list and using \texttt{lapply}}
%-------------------------------------------------------------
