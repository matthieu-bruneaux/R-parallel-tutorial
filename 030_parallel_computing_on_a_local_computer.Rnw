\chapter{Parallel computing on a local computer}
%-----------------------------------------------
...
(Note: the examples given in this section assume a good knowledge of the 
\R\ basics, including loops, linear models and plotting. However, even if you 
only know the basics which were introduced in the previous section, you should 
...
\begin{itemize}
  \item The data must be stored as elements of, e.g.,  a list.
  \item The analysis must be contained into a function that can be applied
        on each element of this list.
\end{itemize}

We have seen a simple example with the \texttt{iris} dataset covering those two
points in the previous section. The next steps are:
\begin{itemize}
  ...
  \item Each core must be given the information needed (i.e. variables and 
        functions).
  \item A parallel version of the appropriate \emph{apply}  function must 
        (e.g. \texttt{parLapply} if \texttt{lapply} and lists are used, 
        \texttt{parSapply} if \texttt{sapply} and vectors are used) distribute 
        the calls of the analysis function to each element of the input data on 
        the different cores.
...
\begin{itemize}
  \item Generate bootstrap resamples, store them in a list and apply the
        ...
        distribution for the bootstrap resamples. Here a list and 
        \texttt{parLapply} can be used.
  \item Store indices in a vector (e.g. from 1 to 10000), and apply the analysis
        function to them. In this case the bootstrap resample generation is done
        within the analysis function, and the memory needs are reduced. Here a
        vector and \texttt{parSapply} can be used.
\end{itemize}



\section{Basic cluster setup: bootstrapping regression models}
%-------------------------------------------------------------
...
head(cars)
str(cars)
# calculate v^2 and store it in the cars data frame
...
# empty list
resamples = list()
# generate the resamples in a for loop
...
<<>>=
library(snow)
# define the number of cores (this should not exceed the number of 
# cores on the local computer used)
n.cores = 3
...
\texttt{parLapply} function instead of the \texttt{lapply} function that we
would have used for a single core run.
...
The parallel steps were performed above. The following steps are only a 
manipulation of the results to put them in a useful format.
...
Now it works. The reason is that we defined \texttt{h} in our current workspace,
but when the cluster is initialized, each node has its own workspace. Only the
list elements it is working on and the function used by \texttt{parLapply} are
sent to each node, and it does not see the global environment of the calling
workspace.
...
Different strategies can be used to export data to a cluster. Variables can
be exported using the \texttt{clusterExport} function as shown here, or can also 
be defined
in the body of the function run on each cluster instead of being defined in the
global environment. Finally, it is also possible to develop a package to store
a project data and function, even though this is more advanced R programming.
If it is the case, the package can be loaded inside the body of the function
applied by the cluster, and all variables contained within the package will be
...
independent stream of random number, otherwise the parallel runs which just
...
need to initialize different seeds on the cores to have different streams.
...
parallel computing could be found with the \texttt{snowFT} package; see also 
