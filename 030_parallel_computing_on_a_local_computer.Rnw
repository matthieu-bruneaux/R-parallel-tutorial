...
\subsection{First example: bootstrapping regression models}
%----------------------------------------------------------
...
$v^2$ and the stopping distance $d$ (if we assumes the cars had the same 
weight).
...
\begin{itemize}
  \item We generate $n$ bootstrap resamples with replacement from the observed
  dataset.
...
\subsubsection{Function for analysis of the resamples}
%-----------------------------------------------------
...
#  are stored in model$coefficients[2])
...
using 'unlist' and dropping the names of the elements.
