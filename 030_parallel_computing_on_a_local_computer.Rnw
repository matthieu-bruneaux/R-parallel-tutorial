\chapter[Parallel computing on a local computer (\texttt{snow} library)]
        {Parallel computing on a local computer (\texttt{snow}
          library)
...
The output of lapply is a list. We can convert it to a vector using
