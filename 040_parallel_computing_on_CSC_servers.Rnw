\section{Parallel computing on CSC servers}
%------------------------------------------
...
\subsection{Preparing the script locally}
%----------------------------------------
...
print(system.time({simulations = parLapply(cluster, index, toss.coin)}))
...
\subsection{Setting up the script on Taito}
%------------------------------------------
...
the simple command \verb+ssh toto@taito.csc.fi+ from a Linux terminal. From the
remote terminal, you can start jobs and examine the results of the jobs. To
move files to and from Taito, the easiest way is to use a SFTP client.



\subsubsection{Copying a file to Taito}
%--------------------------------------
...
\subsubsection{Preparing the sbatch file}
%----------------------------------------



Each job request on Taito is processed by \texttt{sbatch}. To make a request,
we can to wrap the call to the R script in a small script which can be used by
...
\item All the lines starting with \verb+#SBATCH+ are comments which are used
  by \texttt{sbatch} to process the job request.
\item -J: name of the job
\item -o: file where output is redirected (\%J is a placeholder for the job id)
\item -e: file where errors are redirected
\item -t: time request for the job. The job will be killed if it goes beyond 
  that time limit.
\item -n: number of task run by the job
\item --nodes: number of node requested. I have only tried to use one node with
  \texttt{snow}, I don't know if it can handle more than one node.
\item --cpus-per-task: number of cores used by the script. One node on Taito has
  16 cores, so \texttt{snow} can be run with 16 cores specified in the R script.
\item --mem-per-cpu: each node in Taito has 64 Go of memory. If one uses all 16
  cores of a node, then each core can use up to 4000 Mo of memory.
\item --mail-type and --mail-user: useful to get information about when the job
  starts or ends.
...
\subsection{Comparison with a serial job}
%----------------------------------------
...
expected based on the number of cores used (16), but again in this case the
