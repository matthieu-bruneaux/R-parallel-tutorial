\chapter{Parallel computing on CSC servers (\texttt{snow} library)}
%------------------------------------------------------------------
...
\item -o: file to which output is redirected (\%J is a placeholder which will be 
  replaced by the job id during the run).
\item -e: file to which errors are redirected
\item -t: time request for the job. The job will be killed if it goes beyond 
  that time limit. The format is \texttt{hh:mm:ss}.
\item -n: number of task run by the job
\item --nodes: number of node requested. Using \texttt{snow} as it is explained 
  in this tutorial, you cannot use more than one node (but it might be possible
  to do it with \texttt{snow} by creating a different type of cluster). If you
  want to use more than one node, you can read the next section which uses
  \texttt{Rmpi} to run a script with more than one node.
\item --cpus-per-task: number of cores used by the script. One node on Taito has
  16 cores, so \texttt{snow} can be run with at most 16 cores (specified in the 
  R script during the cluster initialization).
